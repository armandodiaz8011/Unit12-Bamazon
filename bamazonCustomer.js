...
function askQuestions() {
    //installed npm inquirer.js to help with this function
    inquirer.prompt([
        {
...
function withdrawProd(prodId, prodQty) {
  connection.query('SELECT * FROM products', function (error, res) {
    ...
    //products are in sql table per assignment instructions
    console.log(prod, "prod was found")
      if(prod.stock_quantity >= prodQty){
        orderComplete(prod, prodId, prodQty)
        connection.end()
      }else{
        console.log("sorry the order has been canceled, there was insufficient stock of this purchase")
        connection.end()
      }
  })
};
